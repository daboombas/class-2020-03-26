...
train %>% group_by(treatment) %>%
  ...
  pivot_wider(names_from = "treatment", values_from = "mean_att_chg") %>% mutate(Difference = Control - Treated)
...
train %>%
  ...
  mutate(Difference = Treated - Control) %>% pull(Difference) %>%
  quantile(c(0.005, 0.995))
```

Bayesian Definition: 99% probability that the true treatment and control difference falls within this interval.

Frequentist Definition: Over many trials, 99% of the sample means will fall within this range.
...
The units for the experiment were the quantifiable numbers of a person's attitude towards immigration. The treatment was witnessing people speaking Spanish in the train. The outcomes were that if you witnessed Spanish in the train, you became less accepting of immigration and more conservative. The causal effect was whether a change in demographics changes a person's attitude towards that specific demographic. The fundamental problem of causal inference is that a specific person cannot be both in the treatment group and the control group so the degree of effect of a cause on a person's attitude cannot be completely determined.
...
```{r}
train %>% ggplot(aes(x = treatment, y = att_chg)) + geom_jitter()
```
